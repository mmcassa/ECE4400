...
#define SMART_TIME 10   // Smart packet dropping period
...
void add2queue(char *argv[],int *share1, int *share2,int *share3, struct Priority *pq) {
    ...
    for(i=0;i<256;i++) {
        smartQ[i].entry_time = clock()+1000;
    ...
    while( i < f_size) {
        ...
        if(checkSmart(smartQ,bVal) == 0) {
            ...
            if (bVal < 30) {
                ...
                for(j=0;j<pq->max_q;j++) {
                    if (qt->packet == -1) {
                        pq->high->packet = bVal;//createPacket(bVal);
                        pq->high->entry_time = clock();
                        j =0;
                        break;
                    }else if (qt->next->packet == -1) {
                        qt->next->packet = bVal;
                        qt->next->entry_time = clock();
                        j =0;
                        break;
                    }
                    qt = qt->next;
        ...
        //while ((getCurrentTimestamp()-starT)*1000 < ARIV_TIME) {};
        sleep(ARIV_TIME);
    }
    i = 1;
    ...
    *share3 = 0;
    ...
    for(j=0;j<3;j++) {
        ...
        for(k=0;k<MAX_QUEUE;k++) {
            if (k!=0) {
                temp = (struct Queue *) create_shared_memory(sizeof(struct Priority));
                temp->packet = -1;
                q = temp;
                q = q->next;
            } else {
                if (j== 0) {
                    pq->high = (struct Queue *) create_shared_memory(sizeof(struct Priority));
                    pq->high->packet = -1;
                    q = pq->high->next;
                } else if (j == 1) {
                    q = pq->med;
                    pq->med = (struct Queue *) create_shared_memory(sizeof(struct Priority));
                    pq->med->packet = -1;
                    q = pq->med->next;
                } else {
                    pq->low = (struct Queue *) create_shared_memory(sizeof(struct Priority));
                    pq->low->packet = -1;
                    q = pq->low->next;
                }
